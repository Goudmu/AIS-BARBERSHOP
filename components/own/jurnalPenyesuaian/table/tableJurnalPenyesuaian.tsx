...
import FormJurnalPenyesuaian from "../form/formJurnalPenyesuaian";

export default function TableJurnapPenyesuaianComponent() {
  const [adjustEntries, setadjustEntries] = useState<IGeneralLedger[]>([]);
  const [trigger, setTrigger] = useState(false);

  const getData = async () => {
    const res = await fetch("/api/adjustledger", { cache: "no-store" });
    ...
    setadjustEntries(newgeneralLedger);
  };
  ...
  return (
    <div className="grid gap-8">
      <FormJurnalPenyesuaian setTrigger={setTrigger} trigger={trigger} />
